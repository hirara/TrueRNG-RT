    ...
    while True:
        ...
        if rng_com_port==None:
            ...
            if mode=='TrueRNG':
                ...
                MeanList[Xpos] = MeanVal
                EntropyList[Xpos] = EntropyVal

                if ListInit == 0:
                    fig = pyplot.figure(figsize=(8,4),dpi=100)
                    fig.suptitle('TrueNG3 mean & entropy')
                    plt1 = fig.add_subplot(211)
                    plt2 = fig.add_subplot(212)
                    move_figure(fig,0,0)
                    plt1.set_xlim(0,255)
                    plt1.set_ylim(120,130)
                    plt2.set_xlim(0,255)
                    plt2.set_ylim(7.9,8.1)
                    # plt1.set_facecolor((1.0,1.0,1.0))
                    d = datetime.datetime.now()
                    filestr = "log" + d.strftime('%Y%m%d%H%M') + ".txt" 
                    print(filestr)
                    fp = open(filestr, 'a')
                ...
                plt1.bar(np.arange(len(MeanList)),MeanList,1, color='k')
                # plt1.set_xlim(0,255)
                plt1.set_title("Mean Values")
                plt2.bar(np.arange(len(EntropyList)), EntropyList,1)
                Xpos = Xpos +1
                if Xpos > 255:
                    Xpos = 0
                

                print("Xpos:"+ str(Xpos) + "  Mean:" + str(MeanVal) + "  Entropy:" + str(EntropyVal))
                d = datetime.datetime.now()
                timestr = d.strftime('%Y/%m/%d/%H:%M:%S') 
                wstr = timestr + ", " + str(Xpos) + ", " + str(MeanVal) +"\n"
                fp.write(wstr)
                pyplot.pause(0.1)
